  ...
  async function handleSubmit(e: Event) {
    ...
    try {
      ...
      setTimeout(() => {
        ...
        // Store in localStorage for persistence if "remember me" is checked
        if (rememberMe) {
...
<div class="min-h-screen bg-neutral-50 py-12 px-4 sm:px-6 lg:px-8 flex flex-col justify-center">
  <div class="max-w-md w-full mx-auto">
    <div class="text-center mb-8">
      ...
      <h1 class="mt-6 text-3xl font-bold text-neutral-900">Sign in to your account</h1>
      <p class="mt-2 text-neutral-600">
        Or <Link to="/register" class="text-primary-600 hover:text-primary-700 font-medium">create a new account</Link>
      </p>
    </div>
      ...
      <form class="space-y-6" on:submit={handleSubmit}>
        ...
        <div class="flex items-center justify-between">
          <div class="flex items-center">
            ...
            />
            <label for="remember-me" class="ml-2 block text-sm text-neutral-700">Remember me</label>
          </div>
          
          <div class="text-sm">
            <Link to="/forgot-password" class="text-primary-600 hover:text-primary-700">Forgot your password?</Link>
          </div>
        </div>
